...
const PersonaChip = styled.button`
  ...
  border-radius: 14px;
  border: 1px solid ${props => (props.active ? 'rgba(var(--accent-rgb), 0.65)' : 'rgba(99, 102, 241, 0.24)')};
  background: ${props => (props.active ? 'rgba(18, 32, 58, 0.95)' : 'rgba(10, 20, 38, 0.85)')};
  color: #e2e9ff;
  text-align: left;
  cursor: pointer;
  transition: transform 0.15s ease, border-color 0.15s ease, box-shadow 0.15s ease;
  box-shadow: ${props => (props.active ? '0 18px 36px rgba(var(--accent-rgb), 0.18)' : '0 12px 28px rgba(7, 15, 30, 0.4)')};
...
export default function SyntheticVoiceover() {
  ...
  const handleVideoUpload = async (event) => {
    ...
    setStatus('Analyzing video speakers... This may take 2-10 minutes depending on video length. Please be patient...');
    ...
    try {
      ...
      // Prefer async analysis + polling to avoid long request timeouts.
      // Hint Transcribe that we expect up to 3 speakers (user report: 3 distinct speakers).
      formData.append('max_speakers', '3');

      const startResponse = await axios.post(`${VOICE_API_BASE}/analyze-video-speakers`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
        timeout: 180000, // upload + job start should be fast; polling handles the long wait
      });

      const jobId = startResponse?.data?.job_id;
      const statusUrl = startResponse?.data?.status_url;
      if (!jobId || !statusUrl) {
        throw new Error(startResponse?.data?.error || 'Speaker analysis did not return a job id.');
      }

      const pollDeadlineMs = Date.now() + 20 * 60 * 1000; // 20 minutes
      let completedPayload = null;
      while (Date.now() < pollDeadlineMs) {
        setStatus('Analyzing video speakers... Transcription in progress (polling).');
        // A single poll can take longer than 30s when Transcribe completes (backend fetches transcript JSON).
        const statusResponse = await axios.get(`${VOICE_API_BASE}${statusUrl}`, { timeout: 180000 });
        const state = statusResponse?.data?.status;
        if (state === 'completed') {
          completedPayload = statusResponse.data;
          break;
        }
        if (state !== 'in_progress' && state !== 'finalizing') {
          const errorMessage = statusResponse?.data?.error || `Unexpected status: ${state}`;
          throw new Error(errorMessage);
        }
        // wait 4s between polls
        await new Promise((resolve) => setTimeout(resolve, 4000));
      }

      if (!completedPayload) {
        throw new Error('Speaker analysis timed out while polling status.');
      }

      const speakersList = completedPayload?.speakers || [];
      setSpeakers(speakersList);
      setScriptEditsDirty(false);

      const transcriptText = (completedPayload?.transcript || '').trim();
  ...
  const inputOptions = [
    ...
    {
      id: 'video',
      title: 'Upload video (multi-speaker)',
      copy: 'Upload a video to identify different speakers and replace their voices with synthetic alternatives.',
    },
  ];
  ...
  setStatus('Unable to craft valid SSML from the current prompt. Try adjusting the inputs.');
      ...
      if (isNetworkError) {
        setStatus('Cannot reach the AI Dubbing service. Ensure it is running (http://localhost:5003) or set REACT_APP_VOICE_API_BASE.');
      } else {
  ...
  const handleSynthesize = async () => {
    ...
    try {
      const response = await axios.post(`${VOICE_API_BASE}/synthesize`, {
        ...
        targetPollyLanguage: targetPollyLanguage,
      });
    ...
    setStatus(`Voiceover is ready.${noticeText} Download or preview the file.`);
      ...
      if (error?.message === 'Network Error') {
        setStatus('Cannot reach the AI Dubbing service. Confirm it is accessible and not blocked by CORS or firewall.');
      } else {
                    ...
                    <div style={{
                      ...
                      <div style={{ marginBottom: '0.75rem' }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                          ...
                          />
                          <span style={{ fontWeight: 600, color: '#e2e9ff' }}>
                            Enable Translation (Translate to different language)
                          </span>
                        </label>
                      </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '0.75rem' }}>
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                const lang = translationLanguages.find(l => l.code === e.target.value);
                                setTargetLanguage(e.target.value);
                                if (lang && lang.polly_codes && lang.polly_codes.length > 0) {
                                  setTargetPollyLanguage(lang.polly_codes[0]);
                                }
                              }}
                              style={{
                                width: '100%',
        ...
        {currentStep === 3 && (
          <Panel ref={voiceSectionRef}>
            ...
            <VoiceList>
              ...
              {/* Translation Options - Available for all input modes */}
              <div style={{
                background: 'rgba(56, 189, 248, 0.08)',
                padding: '1rem',
                borderRadius: '8px',
                ...
                <div style={{ marginBottom: '0.75rem' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                    <input
                      ...
                      style={{ width: '18px', height: '18px', cursor: 'pointer' }}
                    />
                    <span style={{ fontWeight: 600, color: '#e2e9ff', fontSize: '0.95rem' }}>
                      Enable Translation (Translate content to different language)
                    </span>
                  </label>
                  {translateEnabled && (
                    <div style={{ fontSize: '0.85rem', color: '#94a3e6', marginTop: '0.25rem', marginLeft: '1.75rem' }}>
                      Your content will be translated before generating the voiceover
                    </div>
                  )}
                </div>
                ...
                {translateEnabled && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '0.75rem' }}>
                    <div>
                      <Label style={{ fontSize: '0.9rem' }}>Source Language:</Label>
                      <Select
                        value={sourceLanguage}
                        onChange={(e) => setSourceLanguage(e.target.value)}
                      >
                    ...
                    <div>
                      <Label style={{ fontSize: '0.9rem' }}>Target Language:</Label>
                      <Select
                        value={targetLanguage}
                        onChange={(e) => {
                          const lang = translationLanguages.find(l => l.code === e.target.value);
